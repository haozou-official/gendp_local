...
cam_keys = ['camera_0', 'camera_1', 'camera_2', 'camera_3']
...
kin_helper = KinHelper(robot_name='xarm6_with_gripper')
...
for i in tqdm(epi_range):
    ...
    for t in tqdm(range(T)):
        ...
        # visualize robot
        if vis_robot:
            print(f"[Debug] t={t}, qpos.shape={qpos.shape}, qpos={qpos}")
        ...
        visualizer.render()
        save_dir = f"./vis_output/episode_{i}"
        os.makedirs(save_dir, exist_ok=True)
        save_path = f"{save_dir}/frame_{t:04d}.png"
        visualizer.save_image(save_path) 
