...
def load_mesh(robot_name, finger_names=None):
    ...
    if 'panda' in robot_name:
        ...
        STL_FILE_DICT = {
            'left_finger': f"{repo_path}/sapien_env/sapien_env/assets/robot/xarm7_description/xarm_gripper/meshes/left_finger.stl",
            'right_finger': f"{repo_path}/sapien_env/sapien_env/assets/robot/xarm7_description/xarm_gripper/meshes/right_finger.stl",
            'link7': f"{repo_path}/sapien_env/sapien_env/assets/robot/xarm7_description/xarm_gripper/meshes/base_link.stl",
        }

        STL_OFFSET_DICT = {
            'left_finger': np.array([0, 0, 0, 0, 0, 0, 0]),   # adjust if needed
            'right_finger': np.array([0, 0, 0, 0, 0, 0, 0]),  # adjust if needed
            'link7': np.array([0, 0, 0, 0, 0, 0, 0])          # base/palm
        }

        if finger_names is None:
            finger_names = ['left_finger', 'right_finger', 'link7']
    else:
