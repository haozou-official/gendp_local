...
def task_to_cfg(task, manip_obj=None):
    if task == 'hang_mug':
    ...
    elif task == 'cube_picking':
        cfg = OmegaConf.create(
            {
                '_target_': 'sapien_env.rl_env.cube_picking_env.CubePickingRLEnv',
                'use_gui': True,
                'robot_name': 'panda',
                'frame_skip': 10,
                'use_visual_obs': False,
        ...
        policy_cfg = OmegaConf.create(
            {
                '_target_': 'sapien_env.teleop.cube_picking_scripted_policy.SingleArmPolicy',
            }
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('task_name', help='task name, including hang_mug, mug_collect, pen_insertion, cube_picking')
