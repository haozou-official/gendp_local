...
def generate_arm_robot_hand_info() -> Dict[str, ArmRobotInfo]:
    ...
    xarm_path = Path("robot/xarm7_description/")
    ...
    #xarm6_with_gripper = ArmRobotInfo(path=str(xarm_path / "xarm6_with_gripper.urdf"), hand_dof=0, arm_dof=6, palm_name="link6", arm_init_qpos=[0, 0, 0, 0, np.pi / 2, 0])
    #xarm6_with_gripper = ArmRobotInfo(path=str(xarm_path / "xarm6_with_gripper.urdf"), hand_dof=0, arm_dof=6, palm_name="link6", arm_init_qpos=[0, 0, 0, np.pi, np.pi / 2, np.pi])  # V1: Working but facing forward
    #xarm6_with_gripper = ArmRobotInfo(path=str(xarm_path / "xarm6_with_gripper.urdf"), hand_dof=0, arm_dof=6, palm_name="link6", arm_init_qpos=[0, 0, 0, np.pi, np.pi / 2, np.pi / 2])  # V2
    #xarm6_with_gripper = ArmRobotInfo(path=str(xarm_path / "xarm6_with_gripper.urdf"), hand_dof=0, arm_dof=6, palm_name="link6", arm_init_qpos=[0, 0, 0, np.pi, 0, np.pi])  # V3
    #xarm6_with_gripper = ArmRobotInfo(path=str(xarm_path / "xarm6_with_gripper.urdf"), hand_dof=0, arm_dof=6, palm_name="link6", arm_init_qpos=[0, -np.pi / 4, np.pi / 2, -np.pi / 2, 0, np.pi / 2])  # V4
    xarm6_with_gripper = ArmRobotInfo(path=str(xarm_path / "xarm6_with_gripper.urdf"), hand_dof=0, arm_dof=6, palm_name="link6", arm_init_qpos=[0, 0, -np.pi / 4, np.pi, 0, np.pi])  # V5
    
    #xarm7_with_gripper = ArmRobotInfo(path=str(xarm_path / "xarm7_with_gripper.urdf"), hand_dof=0, arm_dof=7, palm_name="link7", arm_init_qpos=[0, 0, -np.pi / 4, np.pi, 0, np.pi, 0])
    ...
    xarm7_with_gripper = ArmRobotInfo(
        path=str(xarm_path / "xarm7_with_gripper.urdf"),
        hand_dof=0,
        arm_dof=7,
        palm_name="link7",
        arm_init_qpos=[0.0, -0.785398, 0.0, 0.523599, 0.0, 1.308997, 0.0]
    )
    info_dict = dict(
        xarm6=xarm6,
...
def load_robot(scene: sapien.Scene, robot_name, disable_self_collision=True) -> sapien.Articulation:
    ...
    print(f"[common_robot_utils] robot_name: {robot_name}")
    robot_file = info.path
    filename = str(package_dir / robot_file)
    print(f"[common_robot_utils] filename: {filename}")
